...
export default function App() {
  ...
  let checkIsLogin = async () => {
    try {
      ...
      if (getTokenId) {
        let response = await axios.get(`https://my-json-server.typicode.com/Winz05/Front-End-Devusers?id=${getTokenId}`)
  ...
  let onLogin = async (inputUsername, inputPassword) => {
    try {
      ...
      let response = await axios.get(
        `https://my-json-server.typicode.com/Winz05/Front-End-Dev/users?username=${inputUsername}&password=${inputPassword}`
      );
