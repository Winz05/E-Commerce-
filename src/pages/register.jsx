  ...
  let onSubmit = async () => {
    try {
      ...
      let checkEmail = await axios.get(
        `https://my-json-server.typicode.com/Winz05/Front-End-Dev/users?email=${inputEmail}`
      );
      let checkUsername = await axios.get(
        `https://my-json-server.typicode.com/Winz05/Front-End-Dev/users?username=${inputUsername}`
      );

      if (checkEmail.data.length === 0 && checkUsername.data.length === 0) {
        await axios.post(
          "https://my-json-server.typicode.com/Winz05/Front-End-Dev/users",
          {
